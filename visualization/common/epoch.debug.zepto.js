...
(function() {
  ...
  Epoch.Events = (function() {
    ...
    Events.prototype.off = function(name, callback) {
      var i, _results;
      if (!Epoch.isObject(this._events[name])) {
        return;
      }
      if (callback == null) {
    ...
    Events.prototype.trigger = function(name) {
      var args, callback, fn, i, _i, _len, _ref, _results;
  ...
  Epoch.Chart.Base = (function(_super) {
    ...
    Base.prototype.extent = function(cmp) {
      return [
        d3.min(this.data, function(layer) {
          return d3.min(layer.values, cmp);
        }), d3.max(this.data, function(layer) {
...
(function() {
  ...
  Epoch.Chart.Plot = (function(_super) {
    ...
    function Plot(options) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        pos = _ref[_i];
        this.margins[pos] = this.options.margins[pos];
        if (!((givenMargins[pos] != null) || this.hasAxis(pos))) {
          this.margins[pos] = 6;
        }
      }
    ...
    Plot.prototype._redrawAxes = function() {
      if (this.hasAxis('bottom')) {
        this.svg.selectAll('.x.axis.bottom').transition().duration(500).ease('linear').call(this.bottomAxis());
      }
      if (this.hasAxis('top')) {
        this.svg.selectAll('.x.axis.top').transition().duration(500).ease('linear').call(this.topAxis());
      }
    ...
    Plot.prototype._drawAxes = function() {
      if (this.hasAxis('bottom')) {
        this.svg.append("g").attr("class", "x axis bottom").attr("transform", "translate(0, " + (this.innerHeight()) + ")").call(this.bottomAxis());
      }
      if (this.hasAxis('top')) {
        this.svg.append("g").attr('class', 'x axis top').call(this.topAxis());
      }
      if (this.hasAxis('left')) {
        this.svg.append("g").attr("class", "y axis left").call(this.leftAxis());
      }
      if (this.hasAxis('right')) {
        this.svg.append('g').attr('class', 'y axis right').attr('transform', "translate(" + (this.innerWidth()) + ", 0)").call(this.rightAxis());
      }
...
(function() {
  ...
  Epoch.Chart.Area = (function(_super) {
    ...
    Area.prototype.y = function() {
      var a, k, layer, v, _i, _len, _ref, _ref1;
      a = [];
      _ref = this.data;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        layer = _ref[_i];
      ...
      return d3.scale.linear().domain([0, d3.max(a)]).range([this.height - this.margins.top - this.margins.bottom, 0]);
    };

    Area.prototype.draw = function() {
      var area, data, layer, stack, x, y, _ref;
      ...
      layer = this.svg.selectAll('.layer').data(this.data, function(d) {
        return d.category;
      });
...
(function() {
  ...
  Epoch.Chart.Bar = (function(_super) {
    var defaults, horizontal_defaults, horizontal_specific;
    ...
    Bar.prototype.x = function() {
      ...
      if (this.options.orientation === 'vertical') {
        return d3.scale.ordinal().domain(Epoch.Util.domain(this.data)).rangeRoundBands([0, this.innerWidth()], this.options.padding.group, this.options.outerPadding.group);
      } else {
    ...
    Bar.prototype.y = function() {
      ...
      if (this.options.orientation === 'vertical') {
        ...
        return d3.scale.ordinal().domain(Epoch.Util.domain(this.data)).rangeRoundBands([0, this.innerHeight()], this.options.padding.group, this.options.outerPadding.group);
      }
    };
    ...
    Bar.prototype._drawVertical = function() {
      ...
      layer = this.svg.selectAll(".layer").data(data, function(d) {
        return d.group;
      });
...
(function() {
  ...
  Epoch.Chart.Line = (function(_super) {
    ...
    Line.prototype.draw = function() {
      var layer, line, x, y, _ref;
      _ref = [this.x(), this.y(), this.line()], x = _ref[0], y = _ref[1], line = _ref[2];
      layer = this.svg.selectAll('.layer').data(this.data, function(d) {
        return d.category;
      });
...
(function() {
  ...
  Epoch.Chart.Pie = (function(_super) {
    ...
    function Pie(options) {
      ...
      this.arc = d3.svg.arc().outerRadius(radius - this.options.margin).innerRadius(this.options.inner);
    ...
    Pie.prototype.draw = function() {
      ...
      arcs = this.svg.selectAll(".arc").data(this.pie(this.data), function(d) {
      ...
      return text = arcs.append("text").attr("transform", (function(_this) {
        return function(d) {
          return "translate(" + (_this.arc.centroid(d)) + ")";
        };
...
(function() {
  ...
  Epoch.Chart.Scatter = (function(_super) {
    ...
    Scatter.prototype.draw = function() {
      var dots, layer, radius, x, y, _ref;
      Scatter.__super__.draw.call(this);
      _ref = [this.x(), this.y()], x = _ref[0], y = _ref[1];
...
(function() {
  ...
  Epoch.Time.Plot = (function(_super) {
    ...
    Plot.prototype.setData = function(data) {
      var classes, copy, i, layer, start, _results;
      this.data = [];
      _results = [];
      for (i in data) {
        layer = data[i];
        ...
        _results.push(this.data.push(copy));
      }
      return _results;
    };
    ...
    Plot.prototype._prepareTimeAxes = function() {
      var axis, i, k, layer, tickInterval, _i, _len, _ref, _ref1, _results;
      if (this.hasAxis('bottom')) {
    ...
    Plot.prototype.push = function(layers) {
      ...
      if (this._queue.length > this._queueSize) {
        this._queue.splice(this._queueSize, this._queue.length - this._queueSize);
      }
      if (this._queue.length === this._queueSize) {
        return false;
      }
    ...
    Plot.prototype._shift = function() {
      ...
      if (this.hasAxis('right')) {
        this.svg.selectAll('.y.axis.right').transition().duration(500).ease('linear').call(this.rightAxis());
      }
  ...
  Epoch.Time.Stack = (function(_super) {
    ...
    Stack.prototype.setData = function(data) {
      var i, layer, y0, _i, _ref, _results;
      Stack.__super__.setData.call(this, data);
      _results = [];
      for (i = _i = 0, _ref = this.data[0].values.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        y0 = 0;
    ...
    Stack.prototype.extent = function() {
      var i, j, max, sum, _i, _j, _ref, _ref1;
      max = 0;
      for (i = _i = 0, _ref = this.data[0].values.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        sum = 0;
        for (j = _j = 0, _ref1 = this.data.length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; j = 0 <= _ref1 ? ++_j : --_j) {
          sum += this.data[j].values[i].y;
        }
        if (sum > max) {
          max = sum;
        }
...
(function() {
  ...
  Epoch.Time.Area = (function(_super) {
    ...
    Area.prototype._drawAreas = function(delta) {
      var args, borderX, entry, firstX, i, k, layer, trans, w, y, _i, _ref, _ref1, _ref2, _results;
      if (delta == null) {
        delta = 0;
      }
    ...
    Area.prototype._drawStrokes = function(delta) {
      var args, entry, firstX, i, k, layer, trans, w, y, _i, _ref, _ref1, _ref2, _results;
      if (delta == null) {
        delta = 0;
      }
      _ref = [this.y(), this.w()], y = _ref[0], w = _ref[1];
    ...
    Area.prototype.draw = function(delta) {
      ...
      return this._drawStrokes(delta);
    };
...
(function() {
  ...
  Epoch.Time.Bar = (function(_super) {
    ...
    Bar.prototype.draw = function(delta) {
      var args, entry, ex, ey, ey0, i, iBoundry, k, layer, trans, w, y, _i, _len, _ref, _ref1, _ref2, _results;
      if (delta == null) {
        delta = 0;
      }
      if (!this.isVisible()) {
      ...
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        ...
        _results.push((function() {
          ...
          while ((--i >= iBoundry) && (--k >= 0)) {
            ...
            _results1.push(this.ctx.strokeRect.apply(this.ctx, args));
          }
          return _results1;
        }).call(this));
      }
  ...
  Epoch.Time.Heatmap = (function(_super) {
    ...
    function Heatmap(options) {
      ...
      if (Epoch.isString(this.options.opacity)) {
        this._colorFn = colorFunctions[this.options.opacity];
        if (this._colorFn == null) {
          Epoch.exception("Unknown coloring function provided '" + this.options.opacity + "'");
        }
      } else if (Epoch.isFunction(this.options.opacity)) {
        this._colorFn = this.options.opacity;
      } else {
    ...
    Heatmap.prototype._prepareEntry = function(entry) {
    ...
    Heatmap.prototype.draw = function(delta) {
      ...
      this.clear();
      return this.ctx.drawImage(this.paint, delta, 0);
    };
...
(function() {
  ...
  Epoch.Time.Line = (function(_super) {
    ...
    Line.prototype.draw = function(delta) {
      var args, entry, i, k, layer, trans, w, y, _i, _len, _ref, _ref1, _ref2, _results;
      if (delta == null) {
        delta = 0;
      }
      this.clear();
